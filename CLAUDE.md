...
3. **Portable export ("Humble Materials")** - Markdown is the "humble material" of this project — wood and copper, not steel and gold. The core constraint: **when users export their data, they get clean, standard Markdown** that looks good in any reader (Obsidian, Logseq, any text editor). The database and internal representation can be as sophisticated as needed — add columns, tables, structured data to make the site feel modern and snappy. But the **export pipeline** filters out anything non-portable and produces `.md` files that just work. Annotation content is stored as Markdown in `contentMd` text columns and rendered via `react-markdown`. Features that touch user content should produce standard Markdown where possible (blockquotes, bold, lists), but if a feature needs richer internal storage to deliver a modern PWA experience, that's fine — just make sure export knows how to flatten it into clean Markdown.
...
- **Hydration safety convention.** Browser-only APIs (`window`, `localStorage`, `caches`, `navigator`, `document`) don't exist during SSR. If a component's render output depends on these, the server HTML and client's first render will differ, causing React to throw a hydration error and re-render the entire tree from scratch. **Convention:** Use `useHydrated()` from `src/hooks/useHydrated.ts` to gate browser-only JSX: `{hydrated && <BrowserOnlyStuff />}`. For state, initialize with safe defaults and load from browser APIs in `useEffect`. **Never** use `typeof window/caches !== "undefined"` as a JSX conditional without `useHydrated`. A scanner test in `src/lib/hydration-safety.test.ts` enforces this — it will fail if unsafe patterns are introduced.
